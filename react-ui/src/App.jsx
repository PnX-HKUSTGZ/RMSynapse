...
export default function App() {
  ...
  const [bases, setBases] = useState({
    ...
    right: { hp: 2800, shield: 800, state: 0 } // 给蓝方也加一点盾展示紫色效果
  });
  ...
  // --- 状态切换逻辑 ---
  const cycleBaseState = (side) => {
    setBases(prev => ({
      ...prev,
      [side]: { ...prev[side], state: (prev[side].state + 1) % 3 }
    }));
  };

  const cycleOutpostState = (side) => {
    setOutposts(prev => {
      const nextState = (prev[side].state + 1) % 6;
      const newHp = nextState >= 3 ? 0 : 750; 
      return { ...prev, [side]: { ...prev[side], state: nextState, hp: newHp } };
    });
  };
  ...
  return (
    <div className="min-h-screen bg-transparent flex flex-col items-center pt-2 relative overflow-hidden font-sans text-white select-none">
      ...
      {/* ================= 顶层核心布局 ================= */}
      <div className="relative z-10 w-full max-w-[1700px] flex flex-col items-center mt-2">
        ...
        <div className="flex items-center justify-center w-full gap-[2px]">
          ...
          {/* 【左侧：前哨站 (红)】 */}
          <div 
            onClick={() => cycleOutpostState('left')}
            className="w-[120px] h-9 glass-panel border-t border-b border-red-500/40 skew-x-[-20deg] flex flex-col relative overflow-hidden shadow-lg cursor-pointer hover:bg-red-900/40 transition-colors"
          >
          ...
          {/* 【左侧：基地 (红血 + 绿盾)】 */}
          <div 
            onClick={() => cycleBaseState('left')}
            className="w-[24vw] max-w-[400px] h-11 glass-panel skew-x-[-20deg] flex items-center relative overflow-hidden shadow-[0_0_15px_rgba(220,38,38,0.15)] border-t border-b border-red-500/50 cursor-pointer hover:bg-red-900/30"
          >
          ...
          {/* 【右侧：基地 (蓝血 + 绿盾)】 */}
          <div 
            onClick={() => cycleBaseState('right')}
            className="w-[24vw] max-w-[400px] h-11 glass-panel skew-x-[20deg] flex items-center relative overflow-hidden shadow-[0_0_15px_rgba(59,130,246,0.15)] border-t border-b border-blue-500/50 cursor-pointer hover:bg-blue-900/30"
          >
            {/* 修复：解耦盾值和血条。血条铺满底层，盾值条作为独立层悬浮在内侧最上方 */}
            <div className="absolute inset-0 scanline-bg opacity-40"></div>
            {/* 主血条 (蓝) - 独立计算百分比 */}
          ...
          {/* 【右侧：前哨站 (蓝)】 */}
          <div 
            onClick={() => cycleOutpostState('right')}
            className="w-[120px] h-9 glass-panel border-t border-b border-blue-500/40 skew-x-[20deg] flex flex-col relative overflow-hidden shadow-lg cursor-pointer hover:bg-blue-900/40 transition-colors"
          >
        ...
        {/* === 第二排：战车血条与经济 === */}
        <div className="flex justify-between items-start w-full px-[5%] mt-1.5 relative z-10">
          ...
          <div className="flex gap-1.5 flex-shrink-0">
            {robots.left.map(robot => {
              ...
              return (
                <div key={robot.id} className={`w-[60px] h-[32px] ultra-glass skew-x-[-20deg] border-b-[2px] ${isDead ? 'border-neutral-600' : 'border-red-500/80'} relative overflow-hidden shadow-md group`}>
                   {/* 底部血量填充 */}
          ...
          <div className="flex-1 flex justify-center mt-[1px]">
          ...
          <div className="flex gap-1.5 flex-shrink-0">
            {robots.right.map(robot => {
              ...
              const isDead = robot.hp === 0;
              return (
                <div key={robot.id} className={`w-[60px] h-[32px] ultra-glass skew-x-[20deg] border-b-[2px] ${isDead ? 'border-neutral-600' : 'border-blue-500/80'} relative overflow-hidden shadow-md group`}>
                   {/* 底部血量填充 */}
