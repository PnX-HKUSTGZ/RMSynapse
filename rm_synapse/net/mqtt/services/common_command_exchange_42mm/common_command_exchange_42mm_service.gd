extends Node
class_name CommonCommandExchange42mmService
...
func _get_adapter():
	if adapter_getter == null:
		if not _logged_missing:
			_log_error("[CommonCommandExchange42mmService] adapter_getter is not set.")
	...
	if adapter == null:
		if not _logged_missing:
			_log_error("[CommonCommandExchange42mmService] ProtocolAdapter not available.")
			_logged_missing = true
		return null
