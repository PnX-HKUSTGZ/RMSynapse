extends Node
class_name SentryCtrlCommandService
...
func request_sentry_ctrl_command(command_id: int, timeout_sec: float = -1.0) -> int:
	if _has_pending():
...
func _on_sentry_ctrl_result(message) -> void:
	...
	if not _has_pending() or message == null:
		return
	if _result_seq <= int(_pending.get("result_seq", -1)):
		return
	if Time.get_ticks_msec() < int(_pending.get("start_msec", 0)):
		return
	if int(message.get_command_id()) != int(_pending.get("command_id", 0)):
		return
	if int(message.get_result_code()) == 0:
		_finish_pending(ErrorCode.OK)
	else:
		_finish_pending(ErrorCode.PROTOCOL_REJECTED)
...
func _send_pending_once() -> bool:
	...
	var data = AdapterTypes.SentryCtrlCommandData.new()
	data.command_id = int(_pending.get("command_id", 0))
	var ret = int(_bound_adapter.send_sentry_ctrl_command(data))
	return ret >= 0
...
func _try_bind_adapter() -> void:
	if adapter_getter == null:
		...
		if not _logged_missing:
			_log_error("[SentryCtrlCommandService] adapter_getter is not set.")
	...
	if adapter == null:
		_disconnect_bound_adapter()
		if not _logged_missing:
			_log_error("[SentryCtrlCommandService] ProtocolAdapter not available.")
			_logged_missing = true
		return
	if not (adapter is Object) or not adapter.has_signal("sentry_ctrl_result"):
		_disconnect_bound_adapter()
		if not _logged_missing:
			_log_error("[SentryCtrlCommandService] Adapter is invalid or missing signal: sentry_ctrl_result")
			_logged_missing = true
		return
