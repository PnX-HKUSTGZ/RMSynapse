extends Node
class_name RobotPerformanceSelectionCommandService

signal request_started(request_id, shooter, chassis, sentry_control)
...
func request_robot_performance_selection(shooter: int, chassis: int, sentry_control: int, timeout_sec: float = -1.0) -> int:
	if _has_pending():
	...
	_pending = {
		...
		"shooter": int(shooter),
		"chassis": int(chassis),
		"sentry_control": int(sentry_control),
		"deadline_msec": _make_deadline_msec(timeout_sec),
		"had_send_success": false,
	}
	_resend_elapsed = 0.0
	emit_signal("request_started", request_id, int(shooter), int(chassis), int(sentry_control))
...
func _on_robot_performance_selection_sync(message) -> void:
	if not _has_pending() or message == null:
		return
	if int(message.get_shooter()) != int(_pending.get("shooter", 0)):
		return
	if int(message.get_chassis()) != int(_pending.get("chassis", 0)):
		return
	if int(message.get_sentry_control()) != int(_pending.get("sentry_control", 0)):
		return
...
func _send_pending_once() -> bool:
	...
	var data = AdapterTypes.RobotPerformanceSelectionCommandData.new()
	data.shooter = int(_pending.get("shooter", 0))
	data.chassis = int(_pending.get("chassis", 0))
	data.sentry_control = int(_pending.get("sentry_control", 0))
	var ret = int(_bound_adapter.send_robot_performance_selection_command(data))
	return ret >= 0
...
func _try_bind_adapter() -> void:
	if adapter_getter == null:
		...
		if not _logged_missing:
			_log_error("[RobotPerformanceSelectionCommandService] adapter_getter is not set.")
	...
	if adapter == null:
		_disconnect_bound_adapter()
		if not _logged_missing:
			_log_error("[RobotPerformanceSelectionCommandService] ProtocolAdapter not available.")
			_logged_missing = true
		return
	if not (adapter is Object) or not adapter.has_signal("robot_performance_selection_sync"):
		_disconnect_bound_adapter()
		if not _logged_missing:
			_log_error("[RobotPerformanceSelectionCommandService] Adapter is invalid or missing signal: robot_performance_selection_sync")
			_logged_missing = true
		return
