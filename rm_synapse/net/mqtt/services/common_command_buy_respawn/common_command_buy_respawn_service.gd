extends Node
class_name CommonCommandBuyRespawnService
...
func _get_adapter():
	if adapter_getter == null:
		if not _logged_missing:
			_log_error("[CommonCommandBuyRespawnService] adapter_getter is not set.")
	...
	if adapter == null:
		if not _logged_missing:
			_log_error("[CommonCommandBuyRespawnService] ProtocolAdapter not available.")
			_logged_missing = true
		return null
