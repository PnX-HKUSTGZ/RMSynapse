extends Node
class_name RuneActivateCommandService

signal request_started(request_id)
...
func request_rune_activate(timeout_sec: float = -1.0) -> int:
	if _has_pending():
	...
	_resend_elapsed = 0.0
	emit_signal("request_started", request_id)
...
func _on_rune_status_sync(message) -> void:
	if not _has_pending() or message == null:
		return
	if int(message.get_rune_status()) == RUNE_ACTIVATED_STATUS:
...
func _send_pending_once() -> bool:
	...
	var data = AdapterTypes.RuneActivateCommandData.new()
	data.activate = 1
	var ret = int(_bound_adapter.send_rune_activate_command(data))
	return ret >= 0
...
func _try_bind_adapter() -> void:
	if adapter_getter == null:
		...
		if not _logged_missing:
			_log_error("[RuneActivateCommandService] adapter_getter is not set.")
	...
	if adapter == null:
		_disconnect_bound_adapter()
		if not _logged_missing:
			_log_error("[RuneActivateCommandService] ProtocolAdapter not available.")
			_logged_missing = true
		return
	if not (adapter is Object) or not adapter.has_signal("rune_status_sync"):
		_disconnect_bound_adapter()
		if not _logged_missing:
			_log_error("[RuneActivateCommandService] Adapter is invalid or missing signal: rune_status_sync")
			_logged_missing = true
		return
