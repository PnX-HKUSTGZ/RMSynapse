extends Node
class_name HeroDeployModeEventCommandService

signal request_started(request_id, mode)
...
func request_hero_deploy_mode(mode: int, timeout_sec: float = -1.0) -> int:
	if _has_pending():
	...
	_resend_elapsed = 0.0
	emit_signal("request_started", request_id, int(mode))
...
func _on_deploy_mode_status_sync(message) -> void:
	if not _has_pending() or message == null:
		return
	if int(message.get_status()) == int(_pending.get("mode", 0)):
...
func _send_pending_once() -> bool:
	...
	var data = AdapterTypes.HeroDeployModeEventCommandData.new()
	data.mode = int(_pending.get("mode", 0))
	var ret = int(_bound_adapter.send_hero_deploy_mode_event_command(data))
	return ret >= 0
...
func _try_bind_adapter() -> void:
	if adapter_getter == null:
		...
		if not _logged_missing:
			_log_error("[HeroDeployModeEventCommandService] adapter_getter is not set.")
	...
	if adapter == null:
		_disconnect_bound_adapter()
		if not _logged_missing:
			_log_error("[HeroDeployModeEventCommandService] ProtocolAdapter not available.")
			_logged_missing = true
		return
	if not (adapter is Object) or not adapter.has_signal("deploy_mode_status_sync"):
		_disconnect_bound_adapter()
		if not _logged_missing:
			_log_error("[HeroDeployModeEventCommandService] Adapter is invalid or missing signal: deploy_mode_status_sync")
			_logged_missing = true
		return
