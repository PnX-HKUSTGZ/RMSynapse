...
@export var map_off_y : float = 0.0 # 右下角向上偏移地图程度 0 - 1
@export var map_off_x : float = 0.0 # 右下角向左偏移地图程度 0 - 1
@export var map_scale : float = 1.0 # 地图缩放比例

#不支持热重载的参数
@export var game_state_path: NodePath = NodePath("/root/MqttNet/GameState")
@export var id_map_path: NodePath = NodePath("/root/IDMap")

@onready var paths_layer = $Panel/PathsLayer
@onready var icons_layer = $Panel/IconsLayer
...
func _process(_delta: float) -> void:
	...
	# 处理地图
	pivot_offset = map_pixel_size
	modulate.a = map_transparency

	scale = Vector2(map_scale, map_scale)

	anchor_right = 1.0 - map_off_x
	anchor_left = 1.0 - map_off_x

	anchor_bottom = 1.0 - map_off_y
	anchor_top = 1.0 - map_off_y
