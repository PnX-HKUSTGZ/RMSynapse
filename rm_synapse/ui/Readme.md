...
- [ ] 事件弹窗绘制
- [ ] 小地图绘制
  - 坐标当前按协议侧 `0~1000` 归一化范围映射到 `28m x 15m` 地图显示比例；`MapClickCmd.map_x/map_y` 也按 `0~1000` 输出。
- [ ] 详细buff绘制
- [ ] 裁判系统等debug界面绘制
- [ ] ESC 网络设置后续确认
...
<!-- - MapClickCmd（触发式发送）：小地图点击交互指令，用于把地图坐标、按键、目标机器人 ID 等信息发给机器人。 -->
...
- `Event.event_id` 按通信协议 V1.3.1 的 1~15 转换为左侧消息，分类为战斗事件、目标事件、机制事件、支援事件、工程装配、系统事件。
- 击杀、摧毁、飞镖命中、装配结果等离散事件不使用固定 `tag` 合并；累计伤害、能量机关状态、基地受击等状态型事件使用固定 `tag` 刷新现有消息。
